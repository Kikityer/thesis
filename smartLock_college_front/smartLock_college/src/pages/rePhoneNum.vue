      ...
      <Layout :style="{padding: '0 30px'}">
        <Content :style="{padding: '24px 25%', margin: '35px 0',Height: 'auto', background: '#fff'}">
          <Card style="margin: 0 0 70px 0; background-color: #fcfdfc">
            ...
            <Form>
              <FormItem >
                <p style="font-size: 15px; font-weight: bold">步骤一：验证当前号码</p>
              </FormItem>
            </Form>
            <Form style="margin: 0 20% "
              ...
              <FormItem label="当前号码">
                <p style="font-size: 15px">{{formValidate.oldPhoneNum}}</p>
              </FormItem>
    ...
    export default {
      ...
      methods:{
        ...
        getPhoneNum(){
          ...
          this.axios({
            method: 'post',
            url: 'http://localhost:8082/revise/phoneNum',
            ...
            if (res.data.message === "no login"){
              ...
              this.formValidate.oldPhoneNum = res.data.msg;
            }
         ...
         */
        sendCode(){
          let phoneNum = this.formValidate.oldPhoneNum;
          this.axios({
            method: 'post',
            url: 'http://localhost:8082/revise/getOldCode',
            params: {phoneNum: phoneNum},
          }).then( res => {
            if(res.data.code === 200){
              this.$Message.success('短信发送成功，请注意查收！');
              localStorage.setItem('oldPhoneNum', phoneNum);
            }
          }).catch(function (error) {
            console.log(error)
          })
        ...
        nextStep(){
          this.$refs.formValidate.validate(async valid => {
            if (valid) {  //表单验证通过
              ...
              this.axios({
                method: 'post',
                url: 'http://localhost:8082/revise/verifyOldNum',
                params: {verifyCode: verifyCode},
              }).then(res => {
                if (res.data.code === 200) {
                  this.$router.push({path:'/rePhoneNum1'});
